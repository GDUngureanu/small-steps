...
const destinationsByStatus = computed(() => {
  ...
  return {
    completed: destinationsWithStatus.value
      .filter(d => d.status === 'epic-adventure-done')
      .sort(sortByPriorityThenAlpha),
    inProgress: destinationsWithStatus.value
      .filter(d => d.status === 'halfway-there')
      .sort(sortByPriorityThenAlpha),
    adventureAwaits: destinationsWithStatus.value
      .filter(d => d.status === 'adventure-awaits')
      .sort(sortByPriorityThenAlpha),
    questForFun: destinationsWithStatus.value
      .filter(d => d.status === 'quest-for-fun')
      .sort(sortByPriorityThenAlpha),
    postponed: destinationsWithStatus.value
        ...
        <div v-if="destination.cities && destination.cities.length > 0" class="accordion accordion-flush">
          <div class="accordion-item border-0">
            <h2 class="accordion-header">
              <button class="accordion-button collapsed bg-transparent border-0 shadow-none p-0" type="button" data-bs-toggle="collapse"
                :data-bs-target="`#halfway-there-${destination.iso}`" aria-expanded="false">
                ...
                <div class="list-group list-group-flush small">
                  <div v-for="city in destination.cities" :key="city.name" class="list-group-item px-0 py-2 border-0">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                      ...
                      <span :class="`badge ${statusBadgeClasses[city.status]}`" v-html="statusLabels[city.status]">
                      </span>
                    </div>
      ...
      <div v-for="destination in destinationsByStatus.adventureAwaits" :key="destination.iso" class="mb-3">
        ...
        <div v-if="destination.cities && destination.cities.length > 0" class="accordion accordion-flush">
          <div class="accordion-item border-0">
            <h2 class="accordion-header">
              <button class="accordion-button collapsed bg-transparent border-0 shadow-none p-0" type="button" data-bs-toggle="collapse"
                :data-bs-target="`#adventure-awaits-${destination.iso}`" aria-expanded="false">
                <div class="d-flex justify-content-between align-items-center w-100">
                  <span>
                    <strong>{{ destination.country }}</strong>
                  </span>
      ...
      <div v-for="destination in destinationsByStatus.questForFun" :key="destination.iso" class="mb-3">
        ...
        <div v-if="destination.cities && destination.cities.length > 0" class="accordion accordion-flush">
          <div class="accordion-item border-0">
            <h2 class="accordion-header">
              <button class="accordion-button collapsed bg-transparent border-0 shadow-none p-0" type="button" data-bs-toggle="collapse"
                :data-bs-target="`#quest-for-fun-${destination.iso}`" aria-expanded="false">
                <div class="d-flex justify-content-between align-items-center w-100">
                  <span>
                    <strong>{{ destination.country }}</strong>
                  </span>
      ...
      <div v-for="destination in destinationsByStatus.completed" :key="destination.iso" class="mb-3">
        ...
        <div v-if="destination.cities && destination.cities.length > 0" class="accordion accordion-flush">
          <div class="accordion-item border-0">
            <h2 class="accordion-header">
              <button class="accordion-button collapsed bg-transparent border-0 shadow-none p-0" type="button" data-bs-toggle="collapse"
                :data-bs-target="`#adventures-done-${destination.iso}`" aria-expanded="false">
                <div class="d-flex justify-content-between align-items-center w-100">
                  <span>
                    <strong>{{ destination.country }}</strong>
                  </span>
