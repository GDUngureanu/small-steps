<script setup>
  ...
  const getScopeCategories = (scope) => {
    ...
    const categories = new Set()
    habitsData.value.habits.filter((habit) => habit.scope === scope && !habit.archived).forEach((habit) => categories.add(habit.category))
  ...
  const scopeData = computed(() => {
    ...
    allScopes.forEach((scope) => {
      ...
      let habits = habitsData.value.habits.filter((habit) => habit.scope === scope && !habit.archived)
...
<template>
  ...
  <ArticleTemplate title="Add Habit Tracker" meta="Aug 6, 2025 by G. D. Ungureanu">
    ...
    <div v-if="formSuccess" class="alert alert-success mb-3" role="alert">Habit created successfully!</div>
    ...
    <form @submit.prevent="addNewHabit" class="d-flex gap-2 align-items-end">
      <div class="flex-grow-1">
        <input v-model="newHabitForm.name" type="text" class="form-control" placeholder="Habit name" maxlength="100" required :disabled="formLoading" />
      </div>
      <div>
      ...
      <div>
        <input v-model="newHabitForm.category" type="text" class="form-control" placeholder="Category" maxlength="50" list="category-suggestions" required :disabled="formLoading" />
        <datalist id="category-suggestions">
          <option v-for="category in allCategories" :key="category" :value="category"></option>
        </datalist>
      </div>
      <button type="submit" class="btn btn-primary" :disabled="formLoading || !newHabitForm.name.trim() || !newHabitForm.category.trim()">
  ...
  <div v-for="(scope, index) in allScopes" :key="scope">
    ...
    <ArticleTemplate :title="scopeConfig[scope].title" meta="Aug 6, 2025 by G. D. Ungureanu">
      ...
      <div v-else-if="scopeData[scope].habits.length > 0 || categoryFilters[scope] !== 'All'" class="small">
        ...
        <div class="d-flex align-items-end pb-2 mb-3 border-bottom">
          <div class="habit-name-column d-flex flex-column gap-1">
            ...
            <div v-if="scopeData[scope].availableCategories.length > 2" class="d-flex flex-column gap-1">
              ...
              <span class="text-muted" style="font-size: 0.65rem" v-if="categoryFilters[scope] !== 'All'"> {{ scopeData[scope].habits.length }} of {{ habitsData.value?.habits?.filter((h) => h.scope === scope && !h.archived).length || 0 }} habits </span>
            </div>
            <div v-else class="text-secondary fw-medium text-center" style="font-size: 0.75rem; padding-top: 1rem">Habits</div>
          </div>
          <div class="d-flex flex-fill gap-2">
            <div v-for="(window, windowIndex) in scopeData[scope].windows.windows" :key="window.intervalId" class="flex-fill text-center p-1 fw-medium text-secondary" :class="{ 'text-primary fw-semibold': windowIndex === scopeData[scope].windows.currentIndex }" style="min-width: 60px; font-size: 1rem">
        ...
        <div v-for="habit in scopeData[scope].habits" :key="habit.id" class="d-flex align-items-center mb-3" role="row" :aria-label="habit.name">
          ...
          <div class="d-flex align-items-center p-2 border rounded habit-name-column">
            ...
            <button @click="deleteHabit(habit.id)" class="btn btn-sm p-1 ms-2 text-danger opacity-75 hover-opacity-100" :aria-label="`Delete habit: ${habit.name}`" title="Delete habit" style="border: none; background: none; font-size: 0.75rem; line-height: 1">
          ...
          <div class="d-flex flex-fill gap-2">
            <div
              v-for="(window, windowIndex) in scopeData[scope].windows.windows"
      ...
      <div v-else class="text-muted">
        ...
        <p v-else>No {{ scopeConfig[scope].plural }} habits found for category "{{ categoryFilters[scope] }}". <button @click="categoryFilters[scope] = 'All'" class="btn btn-link btn-sm p-0 text-decoration-underline">Show all</button></p>
      </div>
    </ArticleTemplate>
  </div>
</template>
